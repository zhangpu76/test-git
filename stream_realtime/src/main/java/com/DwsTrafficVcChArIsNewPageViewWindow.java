        ...
        SingleOutputStreamOperator<TrafficPageViewBean> beanDS = midKeyedDS.map(
                new RichMapFunction<JSONObject, TrafficPageViewBean>() {
                    ...
                    public TrafficPageViewBean map(JSONObject jsonObj) throws Exception {
                        ...
                        Long uvCt = 0L;
                        if (MyStringUtils.isNullOrEmpty(lastVisitDate) || !lastVisitDate.equals(curDate)) {
                            uvCt = 1L;
                            lastVisitDateState.update(lastVisitDate);
                        }
                        String lastPageId = pageJsonObj.getString("last_page_id");
                        Long svCt = MyStringUtils.isNullOrEmpty(lastPageId) ? 1L : 0L;
...
//        beanDS.print();
        //ToDO 3.再次对流中数据进行类型转换jsonObj->统计的实体类对象
        ...
        SingleOutputStreamOperator<TrafficPageViewBean> reduceDS = windowDS.reduce(
                ...
                new WindowFunction<TrafficPageViewBean, TrafficPageViewBean, Tuple4<String, String, String, String>, TimeWindow>() {
                    @Override
                    public void apply(Tuple4<String, String, String, String> stringStringStringStringTuple4, TimeWindow window, Iterable<TrafficPageViewBean> input, Collector<TrafficPageViewBean> collector) throws Exception {
        ...
        reduceDS.map(TrafficPageViewBean::toString)
                .sinkTo(FlinkSinkUtil.getDorisSink("dws_traffic_page_view_window", "dws_traffic_home_detail_page_view_window"));
    }
}
