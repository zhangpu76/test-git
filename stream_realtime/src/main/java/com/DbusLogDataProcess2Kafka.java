...
public class DbusLogDataProcess2Kafka {
    ...
    public static void main(String[] args) {
        ...
        // 手动指定 RocksDB 本地库路径
        System.setProperty("org.rocksdb.nativeLibrary.path", "D:/rocksdb-lib/");
...
//    Kafka 读数据→JSON 转换（脏数据隔离）→按用户分组→状态修正 is_new 字段→日志分流→写入对应 Kafka 主题，实现了用户行为日志的实时清洗、转换与分发。转换与分发
}
