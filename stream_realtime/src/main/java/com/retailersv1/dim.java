...
import com.stream.common.utils.TableProcessDim;
...
//        jsonObjDS.print("jsonObjDS -> ");
        ...
        // 为没有主键的表指定分块键列，这里假设要操作的表是 realtime_v1.base_region，且该表没有主键，指定一个合适的列（如 id）作为分块键
        props.setProperty("scan.incremental.snapshot.chunk.key-column", "realtime_v1.base_region:id");
        ...
        SingleOutputStreamOperator<TableProcessDim> tpDS = mysqlStrDS.map(
                new MapFunction<String, TableProcessDim>() {
                    ...
                    public TableProcessDim map(String jsonStr) throws Exception {
                        ...
                        if ("d".equals(op)) {
                            //对配置表进行了一次刪除操作, 从before属性中获取刪除前的配置信息
