...
public final class KafkaUtils {
    ...
    // 构建KafkaSink（确保方法返回类型是KafkaSink<String>）
    public static KafkaSink<String> buildKafkaSink(String bootstrapServers, String topic) {
        return KafkaSink.<String>builder()
                // 1. 指定Kafka集群地址（必须正确）
                .setBootstrapServers(bootstrapServers)
                // 2. 配置序列化器（必须使用KafkaRecordSerializationSchema）
                .setRecordSerializer(KafkaRecordSerializationSchema.<String>builder()
                        .setTopic(topic) // 目标主题（必须与传入的参数一致）
                        .setValueSerializationSchema(new SimpleStringSchema()) // 用SimpleStringSchema序列化字符串
                        .build()
                )
                // 3. 配置投递语义（至少一次，避免数据丢失）
                .setDeliveryGuarantee(DeliveryGuarantee.AT_LEAST_ONCE)
                .build();
    }

    public static KafkaSource<String> getKafkaSource(String topic, String groupId, String bootstrapServers) {
        return KafkaSource.<String>builder()
                .setBootstrapServers(bootstrapServers)
                .setTopics(topic)
                .setGroupId(groupId)
                .setStartingOffsets(OffsetsInitializer.earliest()) // 从最早位置开始消费
                // 使用正确的反序列化器，适配KafkaRecordDeserializationSchema接口
